export function ArtifactThumb({ kind }: { kind: string }) {
  if (kind === 'art_spend_chart') {
    return (
      <svg viewBox="0 0 110 70" width="180" height="110">
        <circle cx="36" cy="35" r="22" fill="none" stroke="var(--accent)" strokeWidth="7" />
        <circle cx="36" cy="35" r="22" fill="none" stroke="var(--accent-deep)" strokeWidth="7" strokeDasharray="45 180" strokeDashoffset="-20" />
        <rect x="72" y="42" width="7" height="18" fill="var(--accent)" rx="1" />
        <rect x="82" y="32" width="7" height="28" fill="var(--accent-deep)" rx="1" />
        <rect x="92" y="22" width="7" height="38" fill="var(--accent)" rx="1" />
      </svg>
    );
  }
  if (kind === 'art_ap_all') {
    return (
      <svg viewBox="0 0 110 70" width="200" height="120">
        {[0, 1, 2, 3, 4].map(i => (
          <g key={i}>
            <rect x="8" y={10 + i * 11} width="8" height="6" rx="1" fill={i < 2 ? 'var(--accent)' : 'var(--line)'} />
  ...
  if (kind === 'art_crm_flow') {
    return (
      <svg viewBox="0 0 110 70" width="200" height="120">
        <rect x="6" y="26" width="32" height="18" rx="3" fill="var(--accent-soft)" stroke="var(--accent)" strokeWidth="1" />
        <path d="M40 35 L68 35" stroke="var(--accent)" strokeWidth="1.2" />
        <path d="M64 32 L68 35 L64 38" stroke="var(--accent)" strokeWidth="1.2" fill="none" />
    ...
    return (
      <svg viewBox="0 0 110 70" width="200" height="120">
        ...
        <polyline
          ...
          strokeLinecap="round"
        />
        <circle cx="70" cy="54" r="2.5" fill="var(--danger)" />
      </svg>
    );
  }
